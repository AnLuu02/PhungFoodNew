...
export function ConsumerCard({ data }: any) {
  return (
    <Link href={data?.link} target='_blank'>
      ...
      >
        <Box className='relative overflow-hidden' h={200}>
          <Image
        ...
        <Box className='p-4'>
          <Title className='mb-2 line-clamp-2 font-quicksand text-lg font-semibold text-gray-900 transition-colors group-hover:text-mainColor'>
            {data?.title}
          </Title>
          <Text className='mb-3 line-clamp-2 text-sm text-gray-600'>{data?.description}</Text>

          <Box className='mb-3 flex items-center justify-between text-xs text-gray-500'>
