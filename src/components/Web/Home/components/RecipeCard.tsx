  ...
  return (
    ...
    >
      <Box className='relative overflow-hidden rounded-t-md'>
        ...
        <Badge
          variant='light'
          className='absolute right-3 top-3 bg-white/90 text-gray-700 dark:bg-dark-card dark:text-dark-text'
        >
          {data.views} lượt xem
        </Badge>
      ...
      <Stack p={'sm'}>
        <Tooltip label={data.title} withArrow>
          <Text
            ...
            size='md'
            lineClamp={2}
            className='line-clamp-2 leading-relaxed text-gray-900 transition-colors group-hover:text-mainColor dark:text-dark-text'
          >
            {data.title.length < 40 ? `${data.title} - PhungFood dành cho bạn` : data.title}
          </Text>
        </Tooltip>

        <Group justify='space-between' gap='xs' className='text-xs text-gray-500 dark:text-dark-text'>
          <Group gap={4}>
