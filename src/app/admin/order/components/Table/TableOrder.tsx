            ...
            {currentItems.length > 0 ? (
              currentItems.map((order: any) => (
                <Table.Tr key={order.id}>
                  ...
                  <Table.Td className='text-sm'>{new Date(order.createdAt).toLocaleDateString()}</Table.Td>
                  <Table.Td className='text-sm'>
                    <Group className='text-center'>
                      <Badge size='xs' color={getStatusColor(order.status)} p='xs' className='align-items-center flex'>
                        <Flex align='center'>
                  ...
                  <Table.Td className='text-sm'>
                    <Group className='text-center'>
                      {user?.user && (
                        <>
                          <Group>
                            {order.status === LocalOrderStatus.DELIVERED && <SuccessOrderButton id={order.id} />}
                            {order.status === LocalOrderStatus.PENDING && <DeliveryOrderButton id={order.id} />}
