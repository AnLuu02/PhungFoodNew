...
export function CreateManyRoleButton() {
  ...
  const handleExport = async () => {
    ...
    XLSX.writeFile(workbook, `vai_tro - ${new Date().getTime()}.xlsx`);
  };
              ...
              <Table.Tbody>
                {data.map((row, index) => (
                  <Table.Tr key={index}>
                    ...
                    <Table.Td className='text-sm'>{row['Vai trò']}</Table.Td>
...
const mapFieldsPermission: Record<string, string> = {
  Quyền: 'name',
  'Mô tả': 'description'
};
...
export function CreateManyPermissionButton() {
  ...
  const handleImport = async () => {
    try {
      setLoading(true);
      const formatData_: any = formatDataExcel(mapFieldsPermission, data);
