...
export const OverviewUser = () => {
  ...
  const { currentLevel, nextLevel, progressRemainingValue, isMaxLevel } = caculateLevelUser({
    level: user?.level,
                ...
                <Box className='min-w-0'>
                  ...
                  <Box mt='md' maw={430}>
                    <Group justify='space-between' mb={6}>
                      ...
                      >
                        {isMaxLevel ? 'Bạn đã đạt cấp cao nhất' : `Tiến độ lên hạng ${nextLevel?.viName}`}
                      </Text>

                      <Text size='xs' fw={800} style={{ color: currentLevel?.color }}>
                        {progressRemainingValue.toFixed(1)}%
                      </Text>
                    </Group>
