...
export const getRecentActivity = async (db: PrismaClient, input: { startTime?: number; endTime?: number }) => {
  {
    ...
    if (startTime && endTime) {
      ...
      firstRevenue = await db.restaurant.findFirst({ select: { createdAt: true } });
...
export const getRecentActivityApp = async (db: PrismaClient, input: { startTime?: number; endTime?: number }) => {
  const { startTime, endTime } = input;
  let startTimeToDate, endTimeToDate, firstRevenue: any;
  if (startTime && endTime) {
    startTimeToDate = startTime ? new Date(startTime) : undefined;
